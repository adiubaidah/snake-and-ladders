...
export interface GameAreaRef {
  getPlayerRef: (playerId: string) => HTMLDivElement | undefined;
  getCellSize: () => number;
}

const GameAreaComponent = forwardRef<GameAreaRef, GameArea>(
  ({ board, elements, players, boardNumbers }, ref) => {
    const cellSize = 60;
    const boardWidth = board.grid.columns * cellSize;
    const boardHeight = board.grid.rows * cellSize;
    
    // Create refs for player pieces
    const playerRefs = useRef<Map<string, HTMLDivElement>>(new Map());
    
    // Expose methods through ref
    useImperativeHandle(ref, () => ({
      getPlayerRef: (playerId: string) => playerRefs.current.get(playerId),
...
GameAreaComponent.displayName = 'GameAreaComponent';
